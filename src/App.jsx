...
function AppInner() {
  const { role, showAddModal, showLoginModal, showRegisterModal,
          canApprove, canMarkSold, canDeleteListing, canManageUsers,
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {hasAdminAccess && isMobile && mobileSidebarOpen && (
        <div
      ...
      {/* ── Right Content ── */}
      <main style={{
        marginLeft: hasAdminAccess && !isMobile ? `${SIDEBAR_W}px` : '0',
        ...
        {/* Filter bar wrapper */}
        <div style={{ background: 'var(--bg-section)', padding: isMobile ? '12px 16px' : '16px 32px', display: 'flex', justifyContent: 'center' }}>
          <FilterBar />
        </div>
