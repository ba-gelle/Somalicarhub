...
function UserManagement() {
  ...
  const [form, setForm]         = useState({ name: '', email: '', phone: '', password: '', role: 'dealer', permissions: { ...DEFAULT_PERMISSIONS.dealer } })
  const [showPw, setShowPw]     = useState(false)
  ...
  function setF(k, v) {
    if (k === 'role') {
      setForm((f) => ({ ...f, role: v, permissions: { ...DEFAULT_PERMISSIONS[v] } }))
    } else {
      setForm((f) => ({ ...f, [k]: v }))
    }
  ...
  function submitCreate(e) {
    ...
    setForm({ name: '', email: '', phone: '', password: '', role: 'dealer', permissions: { ...DEFAULT_PERMISSIONS.dealer } })
    setCreateOk(true)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      {/* Create dealer / staff — requires createUsers permission */}
      {canCreateUsers ? (
      <div style={{ background: '#E8E8E8', border: '1px solid #ccc', borderRadius: '14px', padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div>
              ...
              <input className="admin-input" placeholder="e.g. Abukar Mohamed" value={form.name} onChange={(e) => setF('name', e.target.value)}
                style={{ ...IF, borderColor: formErr.name ? '#ef4444' : '#334155' }} />
            ...
            <div>
              ...
              <input className="admin-input" type="email" placeholder="email@domain.com" value={form.email} onChange={(e) => setF('email', e.target.value)}
                style={{ ...IF, borderColor: formErr.email ? '#ef4444' : '#334155' }} />
            ...
            <div>
              ...
              <input className="admin-input" type="tel" placeholder="+252 61 xxx xxxx" value={form.phone} onChange={(e) => setF('phone', e.target.value)} style={IF} />
            </div>
            <div>
              <label style={LF}>Role *</label>
              <select value={form.role} onChange={(e) => setF('role', e.target.value)} style={{ ...IF, cursor: 'pointer', background: '#ffffff', color: '#0B1326', border: '1px solid #DE7E16' }}>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <div style={{ position: 'relative' }}>
                <input className="admin-input" type={showPw ? 'text' : 'password'} placeholder="Min. 6 characters" value={form.password} onChange={(e) => setF('password', e.target.value)}
                  style={{ ...IF, paddingRight: '44px', borderColor: formErr.password ? '#ef4444' : '#334155' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '12px 16px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px' }}>
          <UserPlus size={14} color="#94a3b8" />
          <span style={{ color: '#94a3b8', fontSize: '12px', fontFamily: "'DM Sans', sans-serif" }}>Creating user accounts is restricted. Contact an admin to grant this permission.</span>
        </div>
      )}

      {/* All users */}
      <div>
        <h3 style={{ color: '#94a3b8', fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: '15px', margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Users size={15} /> All Users ({allUsers.length})
          <span style={{ color: '#475569', fontSize: '11px', fontWeight: 400 }}>— click <Settings size={10} style={{ verticalAlign: 'middle' }} /> to edit permissions or change password</span>
        </h3>
        <div>
      ...
      {deletedUsers.length > 0 && (
        <div>
          <h3 style={{ color: '#94a3b8', fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: '15px', margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ color: '#475569', fontSize: '11px', fontWeight: 400 }}>— permanently removed after 30 days</span>
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {deletedUsers.map((u) => {
