import { Gauge, LayoutGrid, XCircle, Users, ScrollText, X } from 'lucide-react'
import { useApp } from '../context/AppContext'
import { useWindowSize } from '../hooks/useWindowSize'

export default function AdminSidebar() {
  const { cars, users, currentUser, adminTab, setAdminTab,
          canApprove, canMarkSold, canDeleteListing, canManageUsers, auditLog,
    ...
    <aside style={{
      ...
      {isMobile && (
        <button
          ...
          style={{ position: 'absolute', top: '14px', right: '14px', width: '30px', height: '30px', borderRadius: '8px', background: 'rgba(0,0,0,0.08)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <X size={16} color="#000" />
        </button>
      )}
